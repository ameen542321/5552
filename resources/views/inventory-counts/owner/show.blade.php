...
<div class="max-w-6xl mx-auto space-y-5">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 class="ui-title text-2xl font-bold">{{ $session->referenceCode() }}</h1><p class="ui-text-soft">الحالة: {{ $session->statusLabel() }} — المحاسب: {{ $session->accountant?->name }}</p></div><a class="ui-btn ui-btn-secondary" href="{{ route('user.stores.inventory-counts.pdf', [$store, $session]) }}" target="_blank">تصدير PDF</a></div>
        ...
        @foreach($session->items as $item)
            <x-ui.card>
                <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div><h2 class="ui-title text-lg font-bold">{{ $item->product_name_snapshot }}</h2><p class="ui-text-soft">{{ $item->product_description_snapshot ?: 'لا يوجد وصف' }}</p><p class="ui-text-caption mt-2">الوحدة: {{ ['piece'=>'حبة','kit'=>'طقم','meter'=>'متر','roll'=>'رول','unit'=>'وحدة'][$item->unit_type] ?? $item->unit_type }} — المحاولة: {{ $item->attempt }}</p></div>
                ...
                @else
                    <p class="ui-text-soft mt-3">القرار: {{ ['pending'=>'بانتظار القرار','returned'=>'معاد للمحاسب','approved'=>'معتمد','adjusted_approved'=>'عدّله المالك واعتمده'][$item->decision] ?? $item->decision }} @if($item->owner_quantity !== null)— الكمية النهائية: {{ $item->owner_quantity }}@endif</p>
                @endif
            </x-ui.card>
        @endforeach
    </div>
