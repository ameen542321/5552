...
class AdminPurchaseOrderManagementTest extends TestCase
{
    ...
    public function test_global_limit_service_keeps_one_database_backed_global_setting(): void
    {
        $service = app(\App\Modules\PurchaseOrders\Services\PurchaseOrderLimitService::class);

        $first = $service->global();
        $second = $service->global();

        $this->assertSame($first->id, $second->id);
        $this->assertTrue($first->is_global);
        $this->assertSame(1, PurchaseOrderLimitSetting::where('is_global', true)->count());
    }
}
